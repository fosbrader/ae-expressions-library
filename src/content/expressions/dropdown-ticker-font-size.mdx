...
code: |
  ...
  try {
    var idx = comp("_MASTER").layer("CONTROL").effect("Text")("Menu").value;
...
annotations:
  ...
  - lines: "4-6"
    ...
    description: "Access the dropdown control from <code>_MASTER</code> comp → <code>CONTROL</code> layer → <code>Text</code> effect. The <code>(\"Menu\").value</code> returns the 1-based selection index. Clamping ensures it stays within valid range."
  - lines: "8"
...
2. **Control comp** (`_MASTER`) must have:
