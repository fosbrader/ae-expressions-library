...
tags:
  ...
  - layer-name
code: |
  OnOff = thisComp.layer("CONTROL").effect(thisLayer.name.split("_")[0]+"_"+thisLayer.name.split("_")[1])("Checkbox")
  if (OnOff==1){
  100
  }
  else{
...
annotations:
  - lines: "1"
    ...
    description: "Finds the <code>CONTROL</code> layer in the current comp, then looks for a checkbox effect named after the first two parts of the current layer's name. For layer <code>Player_Card_v2</code>, looks for effect <code>Player_Card</code>."
  - lines: "2-6"
...
This is a simpler version of the cross-comp parse — all controls stay within the same comp.

### Setup

1. Create a control layer named `CONTROL` in your comp
