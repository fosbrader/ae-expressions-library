...
annotations:
  - lines: "1-5"
    ...
    description: "Uses two Point Controls plus sliders for cycle duration and phase offset, all on a `CTRL` layer. Cycle time is clamped to avoid division by zero."
  - lines: "7-9"
...
1. Add Point Controls to `CTRL` named `Start Point` and `End Point`.
2. Add two sliders:
...
- Change `CTRL` to a master control layer name if you centralize controls in another comp.
