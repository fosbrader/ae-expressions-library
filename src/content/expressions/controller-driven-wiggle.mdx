...
annotations:
  - lines: "1-6"
    ...
    description: "Pulls all wiggle parameters from a layer named <code>CONTROL</code>. Amplitude, frequency, octaves, and seed stay editable in one place. Octaves are clamped to an integer minimum of 1."
  - lines: "8-12"
...
1. Add a **Null** named `CONTROL` (or reuse a master `CONTROL` layer in another comp).
2. Apply these effects to `CONTROL`:
...
- If your controls live in another comp, point to them with `comp("Master").layer("CONTROL")` while keeping the layer name `CONTROL`.
